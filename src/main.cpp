...
int selectedIndex = 0;
int windowStart = 0; // Top visible item
...
// Helper to update selectedIndex and handle scrolling window
void updateSelection(int newIndex) {
    ...
    } else if (selectedIndex >= windowStart + 6) {
        windowStart = selectedIndex - 5;
    }
...
uint8_t i2c_read(bool ack) {
    ...
    for (int i = 0; i < 8; i++) {
        digitalWrite(I2C_SCL, HIGH); i2c_delay();
        data <<= 1;
        if (digitalRead(I2C_SDA) == HIGH) data |= 1;
        digitalWrite(I2C_SCL, LOW); i2c_delay();
    }
    pinMode(I2C_SDA, OUTPUT);
...
uint16_t readExpander() {
    ...
    uint16_t data = 0xFFFF;
    ...
    #ifdef USE_MCP23017
        // MCP requires pointing to Register 0x12 (GPIOA) first
        i2c_start();
        ...
        // PCF is simple, just read the bits
        i2c_start();
...
bool isExpanderPressed(uint8_t pin) {
    return !(readExpander() & (1 << pin));
}

void listApps() {
    struct AppFile { String name; time_t time; };
    ...
    while (file) {
        if (!file.isDirectory()) {
            ...
            if (name.endsWith(".bin")) files.push_back({name, file.getLastWrite()}); 
        }
    ...
    std::sort(files.begin(), files.end(), [](const AppFile &a, const AppFile &b) {
        ...
        #else
            return strcasecmp(a.name.c_str(), b.name.c_str()) < 0; // Alphabetical
        #endif
    });
...
void drawUI() {
    ...
    if (!sdMounted) {
        gfx->setCursor(10, 60); gfx->setTextColor(RED);
    ...
    for (int i = windowStart; i < (int)appFiles.size() && i < windowStart + 6; i++) {
        ...
        gfx->setCursor(15, y);
        String disp = appFiles[i];
        if (disp.endsWith(".bin")) disp = disp.substring(0, disp.length() - 4);
        int maxChars = (appFiles[i] == lastApp) ? 19 : 23;
        if (disp.length() > maxChars) disp = disp.substring(0, maxChars - 3) + "...";
        gfx->print(disp);
        if (appFiles[i] == lastApp) { gfx->setTextColor(GREEN); gfx->print(" (*)"); }
        gfx->println();
    }

    // Scroll Indicators
    if (windowStart > 0) {
        gfx->fillTriangle(310, 45, 305, 52, 315, 52, WHITE); // Up arrow
    }
    if (windowStart + 6 < (int)appFiles.size()) {
        gfx->fillTriangle(310, 235, 305, 228, 315, 228, WHITE); // Down arrow
    }
}
...
void flashApp(String fileName) {
    ...
    if (fileName == lastApp) {
        ...
        gfx->setTextSize(3);
        String dispName = fileName; if (dispName.endsWith(".bin")) dispName = dispName.substring(0, dispName.length() - 4);
...
void setup() {
    ...
    if (i2c_write(IO_EXPANDER_ADDRESS << 1)) {
        #ifdef USE_MCP23017
            i2c_write(0x00); i2c_write(0xFF); // IODIRA = All Input
            i2c_stop(); i2c_start(); i2c_write(IO_EXPANDER_ADDRESS << 1);
            i2c_write(0x0C); i2c_write(0xFF); // GPPUA = All Pull-up
            Serial.println("MCP23017 Initialized.");
        #elif defined(USE_PCF8575)
            i2c_write(0xFF); i2c_write(0xFF); // PCF8575: Set all bits high to use as inputs
    ...
    listApps(); 
    updateSelection(0); // Initial draw with scrolling logic
}
